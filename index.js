...
async function weatherFn(cityName) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            $('#wind-speed').html((data.wind.speed * 3.6).toFixed(1));
...
function timeConvert(time24) {
    ...
    var H = +ts.substr(0, 2);
    var h = (H % 12) || 12;
    h = (h < 10) ? ("0" + h) : h;  // leading 0 at the left 
    var ampm = H < 12 ? " AM" : " PM";
...
function degToCompass(num, index) {
    ...
    if (index === -1) { // actual
        ...
        $("#wind-compass").addClass("ms-1 fs-5 bi bi-arrow-" + arr[(val % 16)]);
    } else { // 24 hour
    ...
    for (var i = 0; i < chart.options.data.length; i++) {
        ...
        for (var j = 0; j < dataSeries.dataPoints.length; j++) {
            ...
            if (type === "Rainfall") {
                ...
                if (actY > 30) {
                    dataSeries.dataPoints[j].color = "#FF4400"; // 1000 K
                } else if (actY > 20) {
                    dataSeries.dataPoints[j].color = "#FF6C00"; // 1500 K
                } else if (actY > 10) {
                    dataSeries.dataPoints[j].color = "#FF890E"; // 2000 K
                } else if (actY < 0 && actY >= -5) {
                    dataSeries.dataPoints[j].color = "#71c7ec";
                } else if (actY < -5 && actY >= -10) {
                    dataSeries.dataPoints[j].color = "#1ebbd7";
                } else if (actY < -10) {
                    dataSeries.dataPoints[j].color = "#107dac";
                }
            }
        }
    }
